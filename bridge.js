...
function renderContacts() {
  ...
  return `${pageHead("Contacts", "Search, segment, and move relationships forward.", `<button class="button primary" data-page="add">${icons.plus}<span>Add contact</span></button>`)}
    <div class="toolbar glass"><label class="search">${icons.search}<input id="contactSearch" type="search" value="${escapeHTML(ui.search)}" placeholder="Search contacts" autocomplete="off"></label><div class="segmented">${modeControls}</div><div class="select-wrap">${icons.chart}<select id="sortContacts" aria-label="Sort contacts"><option value="recent" ${ui.sort==="recent"?"selected":""}>Most recent</option><option value="followup" ${ui.sort==="followup"?"selected":""}>Follow-up date</option><option value="interest" ${ui.sort==="interest"?"selected":""}>Interest level</option></select></div></div>

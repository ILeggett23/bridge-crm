...
function stageTitle(stage) { return ({ PQI:"Pre-Qualifying Interview", "QI/P":"Quality Interview / Plan", FUP:"Follow-Up", LA:"Launch", CNA:"", Recommendation:"", "Decision / Follow-Up":"Decision and follow-up", "Order Placed":"Order placed", "Active Customer":"Active customer", "Reorder / Retention":"Reorder and retention" })[stage] ?? stage; }
function stageLabel(stage) { return ({ CNA:"CNA — Customer Needs Assessment", Recommendation:"Proposal" })[stage] || stage; }
...
function renderPipelineGroup(role, contacts) { const stages=PIPELINES[role]; return `<section class="pipeline-role-group"><div class="pipeline-role-head"><span class="eyebrow">${role === "Customer" ? "Customer sales pipeline" : "Prospect pipeline"}</span></div><div class="pipeline-board ${role === "Customer" ? "customer-pipeline" : ""}">${stages.map(stage=>{const group=contacts.filter(c=>c.role===role&&stageFor(c)===stage);return `<div class="pipeline-column glass"><div class="column-head"><div><strong>${escapeHTML(stageLabel(stage))}</strong>${stageTitle(stage)&&stageTitle(stage)!==stageLabel(stage)?`<small class="muted">${escapeHTML(stageTitle(stage))}</small>`:""}</div><span class="pill">${group.length}</span></div>${group.map(c=>`<button class="pipeline-person" data-contact-id="${c.id}"><strong>${escapeHTML(c.fullName)}</strong><div class="muted">${escapeHTML(c.interestLevel)} interest</div></button>`).join("")||'<span class="muted">No contacts</span>'}</div>`}).join("")}</div></section>`; }
...
function stageCheck(stage,title,{type="checkbox",checked=false}={}) { const name=type==="radio"?"pipelineStage":stageInputName(stage); return `<label class="check-tile"><input type="${type}" name="${name}" value="${escapeHTML(stage)}" ${checked?"checked":""}><span><strong>${escapeHTML(stageLabel(stage))}</strong>${title?`<br><small class="muted">${escapeHTML(title)}</small>`:""}</span></label>`; }
function roleStageChecks(role,contact=null) { const type=role==="Customer"?"radio":"checkbox"; return PIPELINES[role].map(stage=>stageCheck(stage,stageTitle(stage)===stageLabel(stage)?"":stageTitle(stage),{type,checked:Boolean(contact?.stages?.[stage])})).join(""); }

  ...
  return `${pageHead("Add New", "Capture a conversation in under a minute.")}
    <form id="addContactForm" class="form-shell">
      <section class="form-section"><h2>Contact</h2><div class="card glass grid form-grid">
        ...
        ${field("Conversation date",`<input name="conversationDate" type="date" max="${todayInput()}" value="${todayInput()}" required>`)}${field("Role",`<select name="role" id="newRole"><option>Prospect</option><option>Customer</option><option>Team</option></select>`)}
        <div data-role-fit-field>${field("Judgement",'<select name="judgement"><option>Good Fit</option><option>Not Good Fit</option></select>')}</div><div data-role-fit-field>${field("Interest",`<select name="interestLevel">${INTERESTS.map(x=>`<option ${x==="Unsure"?"selected":""}>${x}</option>`).join("")}</select>`)}</div>
...
function notificationSettings(s){
  ...
  return `<div class="notification-status"><span class="status-dot ${active?"granted":permission}"></span><div><strong>${title}</strong><small>${detail}</small></div>${actions}</div>${settingsRow("Enable notifications",`<input type="checkbox" name="notificationsEnabled" ${s.notificationsEnabled?"checked":""} ${permission!=="granted"?"disabled":""}>`)}${settingsRow("Follow-up reminders",`<input type="checkbox" name="followUpNotifications" ${s.followUpNotifications?"checked":""}>`)}${settingsRow("Daily conversation reminder",`<input type="checkbox" name="dailyReminderEnabled" ${s.dailyReminderEnabled?"checked":""}>`)}${settingsRow("Daily reminder time",`<input class="compact-time-control" type="time" name="dailyReminderTime" value="${escapeHTML(s.dailyReminderTime||"09:00")}">`,"settings-row-time")}<p class="settings-note">When supported by your browser, the installed Home Screen app can receive hosted follow-up and daily goal reminders while Bridge is closed.</p>`;
}
...
function bindPageEvents(){
  ...
  $('#newRole')?.addEventListener('change',event=>updateNewRoleFields(event.target.value));
...
function scorecardPreviewPNG(scorecard, { format = "preview" } = {}) {
  ...
  const background = dark ? "#0b0f14" : "#f3f5f7";
  const surface = dark ? "#1b2229" : "#ffffff";
  const foreground = dark ? "#f6f8fa" : "#101419";
  const secondary = dark ? "#b9c2cc" : "#5a6571";
  const line = dark ? "rgba(235,240,246,.13)" : "rgba(36,48,61,.12)";
  const accent = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#19aaa7";
  const accentRGB = getComputedStyle(document.documentElement).getPropertyValue("--accent-rgb").trim() || "25,170,167";
  const iconSurface = `rgba(${accentRGB},${dark ? ".20" : ".13"})`;
